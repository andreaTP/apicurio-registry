...
public class GroupsResourceImpl extends AbstractResourceImpl implements GroupsResource {
    ...
    public List<ArtifactReference> getArtifactVersionReferences(String groupId, String artifactId,
        ...
        var gav = VersionExpressionParser.parse(new GA(groupId, artifactId), versionExpression, (ga,
                branchId) -> storage.getArtifactBranchTip(ga, branchId, ArtifactRetrievalBehavior.DEFAULT));
    ...
    public Response getArtifactVersionContent(String groupId, String artifactId, String versionExpression,
        ...
        var gav = VersionExpressionParser.parse(new GA(groupId, artifactId), versionExpression,
                (ga, branchId) -> storage.getArtifactBranchTip(ga, branchId,
    ...
    public void deleteArtifactVersion(String groupId, String artifactId, String version) {
        ...
        var gav = VersionExpressionParser.parse(new GA(groupId, artifactId), version, (ga,
                branchId) -> storage.getArtifactBranchTip(ga, branchId, ArtifactRetrievalBehavior.DEFAULT));
    ...
    public List<ArtifactBranch> listArtifactBranches(String groupId, String artifactId) {
        ...
        return storage.getArtifactBranches(new GA(groupId, artifactId)).entrySet().stream().map(e -> {
            return ArtifactBranch.builder().groupId(groupId).artifactId(artifactId)
                    .branchId(e.getKey().getRawBranchId())
                    .versions(e.getValue().stream().map(GAV::getRawVersionId).collect(toList())).build();
        }).collect(toList());
    }
    ...
    public ArtifactBranch getArtifactBranch(String groupId, String artifactId, String rawBranchId) {
        ...
        return ArtifactBranch.builder().groupId(groupId).artifactId(artifactId).branchId(rawBranchId)
                .versions(storage
                        .getArtifactBranch(new GA(groupId, artifactId), new BranchId(rawBranchId),
                                ArtifactRetrievalBehavior.DEFAULT)
                        .stream().map(GAV::getRawVersionId).collect(toList()))
                .build();

    }

    /**
     * @see io.apicurio.registry.rest.v3.GroupsResource#createOrUpdateArtifactBranch(java.lang.String,
     *      java.lang.String, java.lang.String, java.lang.String)
     */
    @Override
    @Audited(extractParameters = { "0", KEY_GROUP_ID, "1", KEY_ARTIFACT_ID, "2", "branch_id", "3",
            KEY_VERSION }) // TODO
    @Authorized(style = AuthorizedStyle.GroupAndArtifact, level = AuthorizedLevel.Write)
    public ArtifactBranch createOrUpdateArtifactBranch(String groupId, String artifactId, String rawBranchId,
            String version) {
        requireParameter("groupId", groupId);
        requireParameter("artifactId", artifactId);
        requireParameter("branchId", rawBranchId);
        ...
        return ArtifactBranch.builder().groupId(gav.getRawGroupIdWithDefaultString())
                .artifactId(gav.getRawArtifactId()).branchId(branchId.getRawBranchId())
                .versions(storage.getArtifactBranch(gav, branchId, ArtifactRetrievalBehavior.DEFAULT).stream()
    ...
    @Authorized(style = AuthorizedStyle.GroupAndArtifact, level = AuthorizedLevel.Write)
    public ArtifactBranch createOrReplaceArtifactBranch(String groupId, String artifactId, String rawBranchId,
        ...
        var branchId = new BranchId(rawBranchId);
        var versions = branch.getVersions().stream().map(VersionId::new).collect(toList());

        storage.createOrReplaceArtifactBranch(ga, branchId, versions);

        return ArtifactBranch.builder().groupId(ga.getRawGroupIdWithDefaultString())
                .artifactId(ga.getRawArtifactId()).branchId(branchId.getRawBranchId())
                .versions(storage.getArtifactBranch(ga, branchId, ArtifactRetrievalBehavior.DEFAULT).stream()
                        .map(GAV::getRawVersionId).collect(toList()))
                .build();
    }
    ...
    private CreateArtifactResponse handleIfExists(String groupId, String artifactId,
        ...
        switch (ifExists) {
            ...
            // case RETURN:
            // GAV latestGAV = storage.getArtifactBranchTip(new GA(groupId, artifactId), BranchId.LATEST,
            // ArtifactRetrievalBehavior.DEFAULT);
